 ...
 * 
 */
public class CronConfigParser implements ConfigParserInterface {
	protected final String commentRegex = "#.*";
	protected final String directiveRegex = "^(\\W|.*\\s.*\\s.*\\s.*\\s.*\\s*.+)";
	protected final String sectionOpenRegex = "^:::(.+)";

	protected final Matcher commentMatcher = Pattern.compile(commentRegex).matcher("");
	protected final Matcher directiveMatcher = Pattern.compile(directiveRegex).matcher("");
	protected final Matcher sectionOpenMatcher = Pattern.compile(sectionOpenRegex).matcher("");

	public CronConfigParser() {
	}
