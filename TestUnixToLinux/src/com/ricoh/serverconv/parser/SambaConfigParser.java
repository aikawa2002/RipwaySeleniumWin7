 ...
 * 
 */
public class SambaConfigParser implements ConfigParserInterface {
	private final String commentRegex = "#.*";
	private final String kdirectiveRegex = "([^\\s].+)[\\s]*=[\\s]([^ -~｡-ﾟ]+)";
	private final String directiveRegex = "([^\\s].+)[\\s]*=[\\s](.+)";
	...
	private final Matcher kdirectiveMatcher = Pattern.compile(kdirectiveRegex).matcher("");
	...
	public ConfigNode parse(InputStream inputStream) throws IOException {
		...
		while ((line = reader.readLine()) != null) {
			...
			} else if (kdirectiveMatcher.reset(line).find()) {
				String name = kdirectiveMatcher.group(1);
				String content = kdirectiveMatcher.group(2);
