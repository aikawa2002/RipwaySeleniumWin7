 ...
 * 
 */
public class UserAuthConfigParser extends CronConfigParser {
	protected final String authdirectiveRegex = "^(.+)\\s(.+)";
	protected final Matcher authdirectiveMatcher = Pattern.compile(authdirectiveRegex).matcher("");
	...
	public ConfigNode parse(InputStream inputStream) throws IOException {
		...
		while ((line = reader.readLine()) != null) {
			...
			} else if (authdirectiveMatcher.reset(line).find()) {
				String name = authdirectiveMatcher.group(1);
				String content = authdirectiveMatcher.group();
